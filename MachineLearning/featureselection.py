...
def lasso(X, y):
    lasso_select = SelectFromModel(LogisticRegression(C=1, penalty='l1', solver='saga'))
    lasso_select.fit(X, y)
    return lasso_select.get_support(indices=True)


def variance(X, y, n_features):
    where1 = np.where(y==1)[0]
    where0 = np.where(y==0)[0]
    X1 = X[where1, :]
    X0 = X[where0, :]
    sum_1 = X1.sum(axis=0)
    sum_0 = X0.sum(axis=0)
    diff = np.array(abs(sum_1-sum_0))
    order = np.argsort(diff)[-n_features:]
    return order

def prevalence(X, y, n_percentage):
    n = X.shape[0]
    result = []
    for c in range(X.shape[1]):
        length = len(np.where(X[:,c]>0)[0])
        if length/n >= n_percentage:
            result.append(c)
    return result
...
def run_feature_selection(method,X,y):
    ...
    elif method[:8] == 'variance':
        return variance(X,y,int(method[9:]))
    elif method[:10] == 'prevalence':
        return prevalence(X,y,int(method[11:]))