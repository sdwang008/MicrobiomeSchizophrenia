...
def main():
    datafile="normalized-table" #name of the data file in the data folder
    target = "diagnosis" #name of the dependent variable

    #Keep the classification and feature selection methods that you want
    classifiers=['rdforest']
    # classifiers=['logreg', 'xgboost', 'elasticnet', 'knn', 'rdforest', 'naive_bayes', 'svm']
    fselect=['prevalence_90']
    ...
    n_seed = 1 #number of validations
    ...
    df = pd.read_csv(directory_path+datafile+".csv")

    #You can either choose to do a SMOTE run for imbalanced data or a Normal run
    runs.NormalRun(df,directory_path, datafile, target, classifiers, fselect, n_seed, splits)
